...
function loadRuntimeConfig(): SiteConfig | null {
  try {
    const serverHost = import.meta.env.VITE_SERVER_HOST || import.meta.env.VITE_API_BASE_URL?.replace(/\/nfx-storages\/admin\/v3$/, "");
    if (!serverHost) return null;
    return {
      serverHost,
      api: {
        baseURL: import.meta.env.VITE_API_BASE_URL || `${serverHost}/nfx-storages/admin/v3`,
      },
      s3: {
