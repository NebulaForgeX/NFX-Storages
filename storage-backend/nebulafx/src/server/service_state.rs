...
pub(crate) async fn wait_for_shutdown() -> ShutdownSignal {
    ...
    tokio::select! {
        _ = tokio::signal::ctrl_c() => {
            info!("NebulaFX Received Ctrl-C signal");
            ShutdownSignal::CtrlC
        }
        _ = sigint.recv() => {
            info!("NebulaFX Received SIGINT signal");
            ShutdownSignal::Sigint
        }
        _ = sigterm.recv() => {
            info!("NebulaFX Received SIGTERM signal");
    ...
    fn notify_systemd(&self, state: &ServiceState) {
        match state {
            ServiceState::Starting => {
                info!("NebulaFX Service is starting...");
                #[cfg(target_os = "linux")]
                if let Err(e) =
            ...
            ServiceState::Ready => {
                info!("NebulaFX Service is ready");
                notify_systemd("ready");
            }
            ServiceState::Stopping => {
                info!("NebulaFX Service is stopping...");
                notify_systemd("stopping");
            }
            ServiceState::Stopped => {
                info!("NebulaFX Service has stopped");
                #[cfg(target_os = "linux")]
                if let Err(e) =
