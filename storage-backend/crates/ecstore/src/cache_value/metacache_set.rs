...
use nebulafx_filemeta::{MetaCacheEntries, MetaCacheEntry, MetacacheReader, is_io_eof};
            ...
            for (i, r) in readers.iter_mut().enumerate() {
                ...
                let entry = match r.peek().await {
                    ...
                    Err(err) => {
                        ...
                        if err == nebulafx_filemeta::Error::FileNotFound {
                            at_eof += 1;
                            fnf += 1;
                            // warn!("list_path_raw: peek fnf, disk: {}", i);
                            continue;
                        } else if err == nebulafx_filemeta::Error::VolumeNotFound {
                            at_eof += 1;
                            fnf += 1;
                            vnf += 1;
